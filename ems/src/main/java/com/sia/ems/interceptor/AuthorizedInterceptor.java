package com.sia.ems.interceptor;
...
import com.sia.ems.controller.BaseController;
...
public class AuthorizedInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		// 登录请求不拦截
		if (uri.endsWith("ems/") || uri.endsWith("login") || uri.endsWith("login/")) {
			return true;
		}
		...
		String basePath = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + path
				+ "/";
		User loginUser = (User) request.getSession().getAttribute(BaseController.USER_KEY);
